# Full Draft Supervisor v1

This is the supervisor-facing dissertation draft. It preserves the frozen method and formal-v2 evidence while smoothing out project-log phrasing and keeping the narrative closer to dissertation style.
...
This dissertation studies unsignalised intersection control in SUMO using a separated decision pipeline. The method was frozen before the formal experiment and was not redesigned for the dissertation draft.
...
The response parser and validator normalise invalid or missing actions to `WAIT`. This is a defensive interface step, not a behavioural redesign.

### 5.3 Cooperative post-processing

The cooperative post-processor can promote compatible waiting vehicles when the local route conditions permit it. This stage is available in the hybrid controller variants.
...
The formal v2 run manifest records the frozen configuration for each run.
...
The formal experiment is tied to the frozen formal-v2 configuration documented in the repository artefacts.
...
- post-processor intervention count and rate
...
- separate artefact storage for each run.
...
The design is already frozen, the runs are complete, and the dataset is reproducible from repository artefacts. That is enough to write a proper dissertation first draft without modifying the method.
...
This section summarises the formal v2 results from the frozen run artefacts.
...
Saved artefacts do not preserve HTTP status for these failed calls, so the evidence supports a provider-side throttling / rate-limit classification at the client error layer, not a parser or prompt-contract failure.
...
Traffic outcomes are stable across seeds, so seed-level values are reported explicitly.
...
- provider failures dominate the trace, so the post-processor has limited room to influence the final behaviour.
...
- frozen configuration, prompt, model, request configuration, controllers, scales, seeds, planned runs
...
- **OBSERVED RESULT**: what the formal v2 artefacts actually show
...
- The artefacts do not preserve HTTP status for those failed requests.
...
- Because the failure artefacts do not expose HTTP status, the exact provider-side mechanism cannot be proven beyond the recorded exception type.
...
- The experiment uses one unsignalised intersection setting.

- Results cannot be generalised to multi-intersection networks without new evidence.
...
- Provider availability and throttling are part of the system's observed behaviour.
...
## 8. Limited post-processor intervention
...
- that the study generalises to real-world traffic without further validation.
...
- fallback-heavy behaviour dominates the live LLM-bearing runs
...
**Question:** Does cooperative post-processing change the behaviour of raw LLM decisions in a meaningful way?



**Answer:** Cooperative post-processing exists in the pipeline, but formal v2 shows only a rare visible effect. There was only one recorded post-processor intervention in the full formal v2 dataset. The effect is therefore present but small in the available evidence.
...
The formal v2 results are not invalid, but they are strongly mediated by live-provider fallback behaviour. This affects how the results should be interpreted:
...
The dissertation's main contribution is not a claim of universal superiority. Its contribution is a reproducible, traceable comparison of controller architectures for unsignalised intersection control, together with an explicit analysis of how provider availability and fallback behaviour shape the final system behaviour.



That is a useful research contribution because it makes clear where the LLM helps, where it fails, and how much of the final behaviour is attributable to deterministic pipeline stages.
...
Future work could examine how much of the observed traffic behaviour comes from fallback handling. A controlled fallback ablation would help quantify the contribution of deterministic fallback to the final system behaviour.
...
The current dataset contains no safety overrides. Future experiments could include scenario designs that are more likely to trigger the safety verifier, so the dissertation can measure whether safety changes the final decision behaviour in practice.
...
The dissertation shows that a frozen, traceable LLM-assisted decision pipeline can be evaluated systematically in SUMO and can exhibit lower waiting time than a rule-based baseline in the tested scenarios. At the same time, the formal v2 evidence makes clear that live-provider reliability is a first-order validity issue, so the final claim must stay at the level of pipeline behaviour rather than pure LLM intelligence.

## 9. References

# References v1

1. **Dresner, K. and Stone, P. (2008).** *A Multiagent Approach to Autonomous Intersection Management.* Journal of Artificial Intelligence Research, 31, 591-656. Local PDF: `C:\Users\Admin\Desktop\References\A Multiagent Approach to Autonomous Intersection Management.pdf`.

2. **Safarov, K. (2022).** *The impact of autonomous vehicles on traffic performance at an unregulated junction.* PhD thesis, University of Bristol. Local PDF: `C:\Users\Admin\Desktop\References\Final_Copy_2022_04_04_Safarov_K_PhD.pdf`. URL clue in PDF: `http://research-information.bristol.ac.uk`.

3. **Huang, W., Abbeel, P., Pathak, D., and Mordatch, I. (2022).** *Language Models as Zero-Shot Planners: Extracting Actionable Knowledge for Embodied Agents.* arXiv preprint `arXiv:2201.07207v2` (`cs.LG`). Local PDF: `C:\Users\Admin\Desktop\References\Language Models as Zero-Shot Planners.pdf`. URL clue in PDF: `https://huangwl18.github.io/language-planner`.

4. **Driess, D., Xia, F., Sajjadi, M. S. M., Lynch, C., Chowdhery, A., Ichter, B., Wahid, A., Tompson, J., Vuong, Q., Yu, T., Huang, W., Chebotar, Y., Sermanet, P., Duckworth, D., Levine, S., Vanhoucke, V., Hausman, K., Toussaint, M., Greff, K., Zeng, A., Mordatch, I., and Florence, P. (2023).** *PaLM-E: An Embodied Multimodal Language Model.* arXiv preprint `arXiv:2303.03378v1`. Local PDF: `C:\Users\Admin\Desktop\References\PaLM-E embodied multimodal language model.pdf`. URL clue in PDF: `https://palm-e.github.io`.

5. **Cui, C., Ma, Y., Yang, Z., Zhou, Y., Liu, P., Lu, J., Li, L., Chen, Y., Panchal, J. H., Abdelraouf, A., Gupta, R., Han, K., and Wang, Z. (2025).** *Large Language Models for Autonomous Driving (LLM4AD): Concept, Benchmark, Experiments, and Challenges.* arXiv preprint `arXiv:2410.15281v3` (`cs.RO`). Local PDF: `C:\Users\Admin\Desktop\References\LLM Powered Autonomous Driving.pdf`.

6. **Hou, X., Wang, W., Yang, L., Lin, H., Feng, J., Min, H., and Zhao, X. (2025).** *DriveAgent: Multi-Agent Structured Reasoning with LLM and Multimodal Sensor Fusion for Autonomous Driving.* arXiv preprint `arXiv:2505.02123v1` (`cs.RO`). Local PDF: `C:\Users\Admin\Desktop\References\2505.02123v1.pdf`.

7. **Dong, X., Li, J., Xie, J., Yi, Y., Jia, T., Fang, S., Tian, Y., and Hang, P. (2026).** *Large Language Model based Interactive Decision-Making for Autonomous Driving.* arXiv preprint `arXiv:2604.23513v1` (`cs.RO`). Local PDF: `C:\Users\Admin\Desktop\References\2604.23513v1.pdf`.
