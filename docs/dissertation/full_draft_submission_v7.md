...
| Controller | Scale | Provider attempts | Provider successes | Success rate | Fallback count | Fallback rate | Parser success given success | Seed-level successes | Seed-level fallback counts |
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| Raw LLM | 4V | 444 | 26 | 5.86% | 418 | 94.14% | 100% | `[26, 0, 0]` | `[122, 148, 148]` |
| Raw LLM | 8V | 928 | 2 | 0.22% | 926 | 99.78% | 100% | `[1, 1, 0]` | `[289, 334, 303]` |
| Hybrid | 4V | 444 | 22 | 4.95% | 422 | 95.05% | 100% | `[22, 0, 0]` | `[126, 148, 148]` |
| Hybrid | 8V | 928 | 1 | 0.11% | 927 | 99.89% | 100% | `[0, 1, 0]` | `[290, 334, 303]` |
| Hybrid + Safety | 4V | 444 | 22 | 4.95% | 422 | 95.05% | 100% | `[22, 0, 0]` | `[126, 148, 148]` |
| Hybrid + Safety | 8V | 928 | 1 | 0.11% | 927 | 99.89% | 100% | `[0, 0, 1]` | `[290, 335, 302]` |

Notes:

- Rule-based is omitted because it has no live-provider path and therefore no provider attempts, successes, or fallbacks.
- Success and fallback rates are pooled counts over the three seed runs in each controller-scale cell.
- In the raw step records, `fallback_used == fallback_triggered == provider_failure_rows` for every live-provider row in the final evidence boundary.
- The `formal_v4` headline `provider_attempt_rows = 2784` refers to the three live-provider 8V cells only, with 928 attempts per cell.
...
- the corrected 8V boundary records 4 successful provider calls out of 2,784 attempts,
...
4. Provider reliability remains weak, fallback-heavy, and rate-limit constrained; the corrected 8V boundary records 4 successful provider calls out of 2,784 attempts.
...
At the same time, live-provider reliability remains a first-order validity threat, so the final dissertation claim must stay at the level of pipeline behaviour rather than pure LLM intelligence; in the corrected 8V evidence, only 4 of 2,784 provider attempts succeeded.
...
1. **Dresner, K. and Stone, P. (2008).** *A Multiagent Approach to Autonomous Intersection Management.* Journal of Artificial Intelligence Research, 31, 591-656.  
   - Status: `INCOMPLETE_REFERENCE` (local archive did not expose a DOI or URL)  
...
2. **Safarov, K. (2022).** *The impact of autonomous vehicles on traffic performance at an unregulated junction.* PhD thesis, University of Bristol.  
   ...
   - URL clue in PDF: `http://research-information.bristol.ac.uk`
...
3. **Huang, W., Abbeel, P., Pathak, D., and Mordatch, I. (2022).** *Language Models as Zero-Shot Planners: Extracting Actionable Knowledge for Embodied Agents.* arXiv preprint `arXiv:2201.07207v2`.  
   ...
   - URL clue in PDF: `https://huangwl18.github.io/language-planner`
...
4. **Driess, D., Xia, F., Sajjadi, M. S. M., Lynch, C., Chowdhery, A., Ichter, B., Wahid, A., Tompson, J., Vuong, Q., Yu, T., Huang, W., Chebotar, Y., Sermanet, P., Duckworth, D., Levine, S., Vanhoucke, V., Hausman, K., Toussaint, M., Greff, K., Zeng, A., Mordatch, I., and Florence, P. (2023).** *PaLM-E: An Embodied Multimodal Language Model.* arXiv preprint `arXiv:2303.03378v1`.  
   ...
   - URL clue in PDF: `https://palm-e.github.io`
...
## Draft citation policy for the dissertation

- Use the recovered papers only for the claims they actually support.
- Do not invent extra citation details for the incomplete Dresner and Stone entry.
- If the final word-processed dissertation requires a fully normalised bibliography style, the incomplete entry should be verified externally later, but the dissertation draft itself is already citeable from the recovered archive.


# Appendices

## Appendix A. Prompt Contract and Architecture

This appendix records the frozen prompt contract and the staged controller architecture used in the dissertation.

![Figure A1. System architecture of the frozen LLM-assisted intersection controller.](figures/appendix/figure_A1_architecture.png)

*Figure A1. System architecture of the frozen LLM-assisted intersection controller.*

| Field | Evidence-backed content |
|---|---|
| Prompt identifier | P1_BASELINE |
| Input blocks | Traffic state, route conflicts, and policy hints |
| Output schema | Exactly one JSON object with a decisions map |
| Decision labels | PROCEED, WAIT, FREE |
| Hard constraints | One decision per vehicle; outside control zone -> FREE; no prose or markdown |

## Appendix B. Frozen Request Configuration

This appendix records the frozen live-provider request settings used for the final evidence boundary.
...
| Timeout | 30.0 s |
| Max retries | 0 |
| Not set explicitly | temperature, top_p, seed, reasoning_format |

## Appendix C. Experimental Matrix and Evidence Boundary

This appendix summarises the final evidence boundary used in the dissertation.

| Dataset / batch | Role in dissertation |
|---|---|
| formal_v2 valid 4V | Included as the final 4V evidence |
| formal_v4 corrected 8V | Included as the final 8V evidence |
| formal_v2 nominal 8V | Excluded because the raw traces captured only four vehicles |
| formal_v3 | Excluded because two rule-based 8V runs did not complete all arrivals |

## Appendix D. Controller Semantics

The dissertation compares four controller variants that share the same SUMO scenario and decision space.

| Controller | Enabled stages |
|---|---|
| Rule-based | Deterministic interface rule only |
| Raw LLM | Structured prompt, provider request, parsing, validation, and fallback |
| Hybrid | Raw LLM plus cooperative post-processing |
| Hybrid + Safety | Hybrid plus deterministic safety verification |
